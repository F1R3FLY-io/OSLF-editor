...
export class SearchInput extends HTMLElement {
    ...
    attributeChangedCallback(name: string, oldValue: string, newValue: string) {
        if (name === "placeholder" && this.input) {
            this.input.placeholder = newValue || "Categories";
        }
    }

    get placeholder(): string {
        return this.getAttribute("placeholder") || "Categories";
    }
    ...
    render() {
        if (this.shadowRoot) {
            const placeholder = this.getAttribute("placeholder") || "Categories";
            this.shadowRoot.innerHTML = `
				<style>${style}</style>
				<div class="search-container">
					<input type="text" placeholder="${placeholder}"/>
					<button type="button" class="icon-button" data-icon="search">
					...
					<button type="button" class="icon-button clickable hidden" data-icon="clear">✕</button>
				</div>
			`;
        }
    }
    ...
    updateIconState() {
        if (!this.input || !this.searchIcon || !this.clearButton || !this.container) return;
    ...
    setupEventListeners() {
        ...
        this.searchIcon = this.shadowRoot.querySelector('[data-icon="search"]');
        this.clearButton = this.shadowRoot.querySelector('[data-icon="clear"]');
